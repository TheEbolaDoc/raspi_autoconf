#!/bin/bash
user1="chris"
ssh_key_1="ssh-rsa  chris@x"

if [[ ! -d /home/pi/.ssh ]]; then
    mkdir /home/pi/.ssh
fi

if [[ ! -f /home/pi/.ssh/authorized_keys ]]; then
    touch /home/pi/.ssh/authorized_keys
    echo "creating /home/pi/.ssh"
fi

adduser $user1
adduser $user1 sudo 
if [[ ! -d /home/$user1/.ssh ]]; then
    mkdir /home/$user1/.ssh
    echo "creating /home/$user1/.ssh"
fi

if [[ ! -f /home/$user1/.ssh/authorized_keys ]]; then
    touch /home/$user1/.ssh/authorized_keys
fi

echo $ssh_key_1 >> /home/$user1/.ssh/authorized_keys
echo $ssh_key_1 >> /home/pi/.ssh/authorized_keys
echo ./sshd_conf > /etc/ssh/sshd_conf

echo -n "Please enter a hostname: "
read hostname_new
echo $hostname_new > /etc/hostname
sed -i '$ d' /etc/hosts
echo "127.0.1.1    $hostname_new" >> /etc/hosts
echo "new hostname is $hostname_new"

passwd pi

apt update
apt upgrade
apt dist-upgrade
apt install fail2ban vim sl zsh
raspi-config

sh -c "$(wget https://raw.githubusercontent.com/robbyrussell/oh-my-zsh/master/tools/install.sh -O -)"
runuser -l  pi -c "sh -c "$(wget https://raw.githubusercontent.com/robbyrussell/oh-my-zsh/master/tools/install.sh -O -)""
runuser -l  chris -c "sh -c "$(wget https://raw.githubusercontent.com/robbyrussell/oh-my-zsh/master/tools/install.sh -O -)""

echo ./zshrc > /root/.zshrc
echo ./zshrc > /home/pi/.zshrc
echo ./zshrc_chris > /home/$user1/.zshrc